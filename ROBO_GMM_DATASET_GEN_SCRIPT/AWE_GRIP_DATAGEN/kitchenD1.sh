...
#SBATCH -t 2:00:00 # coffee's RDP run of this job (68k frames) took ~8 min; kitchen is 62k frames
#SBATCH --job-name kitchen-d1-awe-grip-pred-gen
...
#
# Pipeline (identical to RDP_DATAGEN/kitchenD1.sh):
...
#      is ~14.6 GB for this task (62,297 frames, ~235 KB/frame).
#
# KITCHEN_D1 frame gaps: 57 of the first 100 source demos are missing frame
# numbers (e.g. demo_1 has no 62.npz). The generator writes one prediction per
# EXISTING source frame under the SAME file name, so the prediction tree keeps
# the gaps. The low-level dataset maps h5 timestep t to the t-th sorted stem,
# which is consistent with both this tree and the contiguously numbered AWE
# keypoint tree next to it.
#
# Checkpoint: CKPT_EPOCH selects the periodic checkpoint of the 2026-09-08
# AWE high-level run. Default 89 (user-selected 2026-09-08). Override at
# submission time:
...
# demo_0..demo_100 only. Lift later with e.g.:
...
FINAL_OCEAN_DIR="${AWE_ROOT}/KITCHEN_D1_GMM_PRED"

# --- checkpoint (CKPT_EPOCH env var, default 89) ----------------------------
CKPT_EPOCH="${CKPT_EPOCH:-89}"
RUN_DIR="${REPO_DIR}/logs/train_KITCHEN_D1_GOAL_SWAP_AWE_GRIP_100demo/2026-09-08/09-55-35"
...
DEST_PRED_DIR="${SCRATCH_ROOT}/KITCHEN_D1_GMM_PRED_AWE"  # generator outputs
