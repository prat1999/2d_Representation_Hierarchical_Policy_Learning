...
#SBATCH --mem=160G            # raised from 80G/120G -- job 3935697 (sibling sweep_tissue script) was cgroup-OOM-killed mid-epoch at 80G despite plenty of free node RAM
#SBATCH -p all
#SBATCH --gres=gpu:1
#SBATCH -C A6000              # grogu-1-15 is a genuine 48GB RTX A6000 node (partition "all", not "dheld" -- dheld's
                               # "A6000"-labeled node, grogu-4-13, is actually a smaller-VRAM card)
#SBATCH -w grogu-1-15
#SBATCH -t 24:00:00
#SBATCH --job-name pusht-d1-approach2-awe-dp-th0.2
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/pusht-d1-approach2-awe-dp-th0.2_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/pusht-d1-approach2-awe-dp-th0.2_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 (GMM-as-auxiliary-loss) on PUSHT_D1  —  goal_source=awe (dp, err_th=0.2)
#
# Sibling of pushtD1_approach2_awe_dp_th0.2.sh / pushtD1_approach2_awe_dp_th0.35.sh --
# same pipeline (raw PushT_npz -> --no_gmm --push_t h5 conversion -> stage ->
# inject extra AWE-dp goals -> train); only SOURCE_TAG/EXTRA_GOALS_DIR (the
# threshold) differs. All three read/write the SAME LOCAL_NO_GMM_H5_DIR cache
# (h5 generation doesn't depend on threshold), guarded by flock below so
# launching all three at once doesn't race the conversion step.
#
#   NUM_DEMOS=206 sbatch this_script.sh
# ===========================================================================
...
# --- paths -----------------------------------------------------------------
SRC_NPZ_DIR="/project_data/held/teswaram/data/D1/PushT/PushT_npz"
EXTRA_GOALS_DIR="/project_data/held/teswaram/data/D1/PushT/EXTRA_KEYPOINTS_awe-dp-th0.2/PushT_npz"
...
# --- generate local NO_GMM h5 pool if not already cached (flock-guarded: --
# shared across the three awe_dp threshold siblings, which may be submitted
# together) -------------------------------------------------------------------
...
echo "[inject] ensuring obs/goal_gripper_pts_awe exists in staged demos (from EXTRA_KEYPOINTS_awe-dp-th0.2)"
(
    cd "${REPO_DIR}"
    USE_TF=0 \
    GIT_LFS_SKIP_SMUDGE=1 \
...
RUN_NAME="PushT_D1_APPROACH2_${SOURCE_TAG}_c1_${C1}_${staged_count}demo_dinov2_DIT_grogu115_a6000"
