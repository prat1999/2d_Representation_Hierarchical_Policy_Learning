...
#SBATCH --mem=160G            # bumped from 80G after job 3937301 was oom-killed (cgroup) at epoch 57 -- node grogu-4-23
                               # is hard-pinned by all RATE_ABLATIONS scripts (-w below), and other jobs (coffee-prep
                               # rate ablations) landed on it concurrently; node has ~1.1TB RAM so this has headroom
#SBATCH -p dheld
#SBATCH --gres=gpu:1
#SBATCH -w grogu-4-23         # pin to the RTX 6000 Blackwell node
#SBATCH -C 6000Blackwell      # pin to the RTX 6000 Blackwell node
#SBATCH -t 48:00:00
#SBATCH --job-name hammer-cleanup-d1-approach2-awe-greedy-th0.35-grip-5hz
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/hammer-cleanup-d1-approach2-awe-greedy-th0.35-grip-5hz_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/hammer-cleanup-d1-approach2-awe-greedy-th0.35-grip-5hz_job_%j.err
...
# entirely: /project_data/held/teswaram/data/rate_ablations/h5/hammer_cleanup_d1_5hz/
...
# --- paths -------------------------------------------------------------------
RATE_H5_DIR="/project_data/held/teswaram/data/rate_ablations/h5/hammer_cleanup_d1_5hz"
EXTRA_GOALS_DIR="/project_data/held/teswaram/data/rate_ablations/npz/EXTRA_KEYPOINTS_awe-greedy-th0.35-grip/hammer_cleanup_d1_5hz"
