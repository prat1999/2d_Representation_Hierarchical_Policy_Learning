...
#SBATCH --cpus-per-task=16    # matches dataloader.num_workers=16 below -- was 12, which under-provisioned the 16 workers and (combined with 3 co-located jobs on grogu-1-15) triggered a cgroup OOM-kill at the start of epoch 1 in job 3935329
#SBATCH --mem=80G             # explicit floor above DefMemPerCPU(3.5G)*cpus, so worker/page-cache spikes don't hit the cgroup limit again
#SBATCH -p all
#SBATCH --gres=gpu:1
...
#SBATCH --job-name kitchen-d1-approach2-awe-greedy-th0.35-grip-grogu
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/kitchen-d1-approach2-awe-greedy-th0.35-grip-grogu_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/kitchen-d1-approach2-awe-greedy-th0.35-grip-grogu_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 (GMM-as-auxiliary-loss) on KITCHEN_D1  —  goal_source=awe (greedy, err_th=0.35, WITH gripper)
# grogu cluster / dheld partition (8x A6000 48GB per node). Sibling of
# kitchenD1_approach2_awe_greedy_th0.35_grogu.sh (the "_nogrip" variant) --
...
RUN_NAME="kitchen_D1_APPROACH2_${SOURCE_TAG}_c1_${C1}_${staged_count}demo_dinov2_DIT_grogu"
