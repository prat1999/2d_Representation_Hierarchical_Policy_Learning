...
#SBATCH -t 24:00:00
#SBATCH --job-name kitchen-d1-approach2-auxregression-absolute-c1-1-actionmode-absolute-grogu423-blackwell
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/kitchen-d1-approach2-auxregression-absolute-c1-1-actionmode-absolute-grogu423-blackwell_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/kitchen-d1-approach2-auxregression-absolute-c1-1-actionmode-absolute-grogu423-blackwell_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2, REGRESSION auxiliary head on KITCHEN_D1 -- ACTION_MODE ABLATION
#   goal_source=awe (greedy, err_th=0.35, WITH gripper)
#   aux_head_type=regression, aux_regression_frame=ABSOLUTE, c1=1.0
#   action_mode=ABSOLUTE   <-- the ONLY difference from
#     kitchenD1_approach2_auxregression_absolute_c1_1_grogu423_blackwell.sh
#     (that sibling uses the repo default action_mode=hybrid_delta and is
#     already fully trained + evaluated: epoch_99 pooled success 50.7%,
#     see /data/theya/models/approach2_models/auxhead_regression_ablation/
#     00.58.23_kitchen_D1_..._absolute_c1_1.0_.../evaluations/)
#
# Motivation: Table 1 of the Equivariant Diffusion Policy paper (Wang et al.,
# arXiv:2407.01812) shows plain (non-equivariant) Diffusion Policy baselines
# (DiffPo-C/DiffPo-T) trained with ABSOLUTE pose actions consistently beating
# their own RELATIVE-action twins across every MimicGen/robosuite task in
# the benchmark -- including Kitchen D1, Hammer Cleanup D1 and Coffee
# Preparation D1, the exact same task family used here. E.g. DiffPo-C on
# Kitchen D1 @100 demos: Abs=67 vs Rel=42.
#
# The mechanism plausibly transfers to Approach 2:
#   - Diffusion/flow-matching policies predict a whole action CHUNK per
#     inference call, open-loop until the next replan. With delta/hybrid
#     actions, a bad step shifts the reference frame for every later step
#     WITHIN that same chunk (intra-chunk compounding). Absolute targets
#     don't have this -- each step is an independent, self-correcting
#     target anchored to the observed world/robot state.
#   - Delta actions are usually favored for generalizing across varying
#     robot-base/camera frames -- but robosuite/MimicGen keeps the base and
#     camera extrinsics FIXED every episode (only object placement is
#     randomized), so that generalization argument doesn't apply here,
#     while the compounding-error cost of delta actions still does.
#
...
# commanded OSC pose target, with its own normalizer branch) -- it has just
# never been run on this task family before. This is a from-scratch training
# run (no checkpoint reuse possible: the action head's output dimensionality
# and target distribution differ from hybrid_delta).
#
# Everything else -- data pool, EXTRA_KEYPOINTS tree, staging, injection,
# node pinning, batch size, checkpointing, aux head settings -- is
# inherited unchanged from the hybrid_delta sibling, so the comparison is a
# clean one-variable (action_mode) change.
# ===========================================================================
...
echo "[config] goal_source=${GOAL_SOURCE} (${SOURCE_TAG}), aux_head=${AUX_HEAD_TYPE} (${AUX_REGRESSION_FRAME}), c1=${C1}, action_mode=${ACTION_MODE}, NUM_DEMOS cap=${NUM_DEMOS}"
...
DEST_DATA_DIR="${SCRATCH_ROOT}/Kitchen_D1_Approach2_${SOURCE_TAG}_${AUX_TAG}_actionmode_${ACTION_MODE}"
...
RUN_NAME="kitchen_D1_APPROACH2_${SOURCE_TAG}_${AUX_TAG}_c1_${C1}_actionmode_${ACTION_MODE}_${staged_count}demo_dinov2_DIT_grogu423_blackwell"
