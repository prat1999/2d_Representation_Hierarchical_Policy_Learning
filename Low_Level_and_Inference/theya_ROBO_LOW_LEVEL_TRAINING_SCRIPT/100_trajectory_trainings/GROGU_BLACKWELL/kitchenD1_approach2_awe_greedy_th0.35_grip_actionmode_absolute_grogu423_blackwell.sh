...
#SBATCH -t 24:00:00
#SBATCH --job-name kitchen-d1-approach2-awe-greedy-th0.35-grip-actionmode-absolute-grogu423-blackwell
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/kitchen-d1-approach2-awe-greedy-th0.35-grip-actionmode-absolute-grogu423-blackwell_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/kitchen-d1-approach2-awe-greedy-th0.35-grip-actionmode-absolute-grogu423-blackwell_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 (GMM-as-auxiliary-loss) on KITCHEN_D1 -- ACTION_MODE ABLATION
#   goal_source=awe (greedy, err_th=0.35, WITH gripper), aux_head=gmm (default), c1=0.1
#   action_mode=ABSOLUTE   <-- the ONLY difference from
#     kitchenD1_approach2_awe_greedy_th0.35_grip_grogu423_blackwell.sh
#     (that sibling uses the repo default action_mode=hybrid_delta and is
#     already fully trained + evaluated locally:
#       epoch_20=56.0% epoch_40=62.0% epoch_60=60.7% epoch_80=58.0% epoch_99=59.3%
#     see /data/theya/models/approach2_models/
#     15.26.28_kitchen_D1_APPROACH2_awe_greedy_th0.35_grip_c1_0.1_..._kitchen_goal_gmm_aux/)
#
# This is the plain-GMM-aux twin of
# kitchenD1_approach2_auxregression_absolute_c1_1_actionmode_absolute_grogu423_blackwell.sh
# (which used aux_head_type=regression instead) -- run instead per request,
# to test the action_mode=absolute hypothesis against the STANDARD Approach 2
# recipe (GMM auxiliary loss) rather than the newer, less-used regression head.
#
...
# trained with ABSOLUTE pose actions consistently beating their own
# RELATIVE-action twins across the MimicGen/robosuite task family used here
# (Kitchen D1 @100 demos: Abs=67 vs Rel=42). robosuite/MimicGen keeps the
# robot base and camera extrinsics FIXED every episode (only object
# placement randomizes), which is exactly the regime where plain absolute
# pose prediction is expected to outperform delta-style actions -- diffusion
# policies predict a whole action chunk per inference call, and with
# delta/hybrid actions an error at one step shifts the reference frame for
# every later step within that same open-loop chunk, whereas each absolute
# target is independently anchored to the observed world/robot state.
#
...
# commanded OSC pose target, with its own normalizer branch, requires
# 'state' in shape_meta.obs -- present here) -- it has just never been run
# on this task family before. From-scratch training run: no checkpoint
# reuse possible, the action head's output distribution differs from
# hybrid_delta.
#
...
DEST_DATA_DIR="${SCRATCH_ROOT}/Kitchen_D1_Approach2_${SOURCE_TAG}_actionmode_${ACTION_MODE}"
...
RUN_NAME="kitchen_D1_APPROACH2_${SOURCE_TAG}_c1_${C1}_actionmode_${ACTION_MODE}_${staged_count}demo_dinov2_DIT_grogu423_blackwell"
