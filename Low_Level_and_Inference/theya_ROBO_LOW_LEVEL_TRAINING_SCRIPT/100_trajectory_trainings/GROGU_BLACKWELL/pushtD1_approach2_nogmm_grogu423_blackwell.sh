...
#SBATCH -w grogu-4-23
#SBATCH -t 24:00:00
#SBATCH --job-name pusht-d1-approach2-nogmm-grogu423-blackwell
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/pusht-d1-approach2-nogmm-grogu423-blackwell_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/pusht-d1-approach2-nogmm-grogu423-blackwell_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 *CONTROL ARM* (arm B) on PUSHT_D1
#   -> NO auxiliary loss at all; the auxiliary head is NOT constructed.
#
# Architecture-matched control for the PushT_D1 Approach-2 treatment arms
# (pushtD1_approach2_awe_greedy_th0.35.sh, GMM c1=0.1, and the
# GROGU_BLACKWELL/pushtD1_approach2_auxregression_* regression arms, c1=10):
# goal removed from the DiT input, same DINOv2 RoPE4D grounded trunk. The ONLY
# difference is the auxiliary head:
#
...
# forced to None at policy.py:146), so there are no auxiliary parameters in the
# model at all and compute_loss() returns fm_loss without ever reading the goal.
# This is what makes the treatment arms interpretable.
#
...
# obs/goal_gripper_pts (the native field already in the PUSH_T_D1 h5 pool,
# declared in the task yaml's shape_meta so n_keypoints can be read from it) --
# the policy simply never touches it.
#
# Demo count and epochs match the treatment arms so the control is comparable:
# staging caps at NUM_DEMOS by numeric demo index, same as the siblings.
#
#   NUM_DEMOS=100 sbatch this_script.sh     # subset to demo_0..demo_99
#   NUM_EPOCHS=100 sbatch this_script.sh
# ===========================================================================
...
echo "[config] arm=${ARM_TAG} (aux_gmm_loss_weight=null, no aux head), NUM_DEMOS cap=${NUM_DEMOS}, epochs=${NUM_EPOCHS}"
...
DEST_DATA_DIR="${SCRATCH_ROOT}/PushT_D1_Approach2_${ARM_TAG}"
...
# Same numeric-index cap as the treatment arms, so the control trains on the
# identical demo subset (demo_0..demo_$((NUM_DEMOS-1))) rather than a
# lexicographic slice.
...
RUN_NAME="PushT_D1_APPROACH2_${ARM_TAG}_${staged_count}demo_dinov2_DIT_grogu423_blackwell"
