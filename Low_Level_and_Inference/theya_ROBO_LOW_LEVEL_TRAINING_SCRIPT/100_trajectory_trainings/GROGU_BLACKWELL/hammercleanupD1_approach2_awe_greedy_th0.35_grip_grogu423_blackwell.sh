...
#SBATCH --mem=80G             # explicit floor above DefMemPerCPU(3.5G)*cpus, so worker/page-cache spikes don't hit the cgroup limit -- the two prior runs of the A6000 sibling script (jobs 3935270, 3935331) predate this floor and were killed by mkdir-on-/local (missing SCRATCH_ROOT dir perms) and a cgroup OOM respectively
#SBATCH -p dheld
...
#SBATCH -t 24:00:00
#SBATCH --job-name hammer-cleanup-d1-approach2-awe-greedy-th0.35-grip-grogu423-blackwell
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/hammer-cleanup-d1-approach2-awe-greedy-th0.35-grip-grogu423-blackwell_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/hammer-cleanup-d1-approach2-awe-greedy-th0.35-grip-grogu423-blackwell_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 (GMM-as-auxiliary-loss) on HAMMER_CLEANUP_D1  —  goal_source=awe (greedy, err_th=0.35, WITH gripper)
# Pinned to grogu-4-23 (8x RTX 6000 Blackwell). Sibling of
# ../GROGU_LOCAL_A6000/hammercleanupD1_approach2_awe_greedy_th0.35_grip_grogu.sh --
# same pipeline, only the SLURM node/partition/constraint selection differs.
#
# Prior runs of the A6000 sibling script both failed before reaching a
# stable epoch:
#   - job 3935270: crashed on `mkdir -p /local/slurm-<id>/local` (permission
#     denied) -- an older revision of the script rooted node-local scratch
#     under `/local` instead of `/scratch/teswaram`. Already fixed below
#     (SCRATCH_ROOT uses /scratch/teswaram/slurm-${SLURM_JOB_ID}/local).
#   - job 3935331: cgroup OOM-killed during epoch 0 (MaxRSS ~11.7G against a
#     job that had only the sacct-default 3.5G/cpu, i.e. an older revision
#     without an explicit --mem floor). Already fixed below (--mem=80G,
#     --cpus-per-task=16 matching dataloader.num_workers=16).
# ===========================================================================
...
#     101 demos for HAMMER_CLEANUP_D1.
#   - Blackwell 6000 cards have well over 48GB VRAM, so the A6000 sibling's
#     batch_size=128 sizing (~34 GiB measured) has comfortable headroom here
#     too -- left unchanged for reproducibility with the other Approach2 runs.
#
...
# --- paths -----------------------------------------------------------------
SRC_NPZ_DIR="/project_data/held/teswaram/data/D1/HAMMER_CLEANUP_D1"
EXTRA_GOALS_DIR="/project_data/held/teswaram/data/D1/EXTRA_KEYPOINTS_awe-greedy-th0.35-grip/HAMMER_CLEANUP_D1"
LOCAL_NO_GMM_H5_DIR="/project_data/held/teswaram/data/D1/NO_GMM_preds/HAMMER_CLEANUP_D1"
...
DEST_DATA_DIR="${SCRATCH_ROOT}/Hammer_Cleanup_D1_Approach2_${SOURCE_TAG}"
...
RUN_NAME="Hammer_Cleanup_D1_APPROACH2_${SOURCE_TAG}_c1_${C1}_${staged_count}demo_dinov2_DIT_grogu423_blackwell"
