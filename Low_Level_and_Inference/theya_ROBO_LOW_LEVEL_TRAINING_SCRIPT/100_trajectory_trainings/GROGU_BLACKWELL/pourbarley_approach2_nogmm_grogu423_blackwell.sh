...
#SBATCH --job-name pourbarley-d1-approach2-nogmm-grogu423-blackwell
#SBATCH -o /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/pourbarley-d1-approach2-nogmm-grogu423-blackwell_job_%j.out
#SBATCH -e /home/teswaram/code/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/pourbarley-d1-approach2-nogmm-grogu423-blackwell_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 *CONTROL ARM* on the real-robot Franka POUR_BARLEY task (D1)
...
# pour_barley_h5 demos already carry obs/goal_gripper_pts, which the dataset
...
# treatment run -- for that, use POUR_BARLEY_D2_TASK/Approach2/pour_barley_approach2_awe_greedy_th0.35.sh
# (or its grogu sibling), which injects obs/goal_gripper_pts_awe from the
# EXTRA_KEYPOINTS tree and passes goal_source=awe.
#
# The pour_barley h5 tree is produced upstream already cropped/resized to the
# 256x256 shape pour_barley_goal_gmm_aux.yaml expects, so there is no --no_gmm
# conversion step here: this script only stages and trains.
#
# Usage:
#   sbatch pourbarley_approach2_nogmm_grogu423_blackwell.sh
#       -> control arm: no GMM head
#   USE_GMM_AUX=1 sbatch pourbarley_approach2_nogmm_grogu423_blackwell.sh
...
DEST_DATA_DIR="${SCRATCH_ROOT}/PourBarley_D1_${ARM_TAG}"
...
RUN_NAME="PourBarley_D1_${ARM_TAG}_${staged_count}demo_dinov2_DIT_grogu423_blackwell"
